...
export default function ProductListPage() {
  ...
  function deleteItem(item) {
    api_client.delete(`products/${item}`).then(() => {
    ...
    return (
      <Swipeable renderLeftActions={() => getLeftContent(item)}>
        ...
        >
            ...
            <Text style={styles.title}>Name: <Text style={styles.data}> {item?.name}</Text></Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginBottom: 20, padding: 15 }}>
        ...
        <TouchableOpacity style={styles.buttonToAdd} onPress={() => navigation.navigate('ProductForm')}>
